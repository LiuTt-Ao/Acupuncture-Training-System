...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	if (!m_splitter.CreateStatic(this, 1, 2))//切割客服区为1行两列
	{
		return FALSE;
	}

	if (!m_splitter.CreateView(0, 0, RUNTIME_CLASS(CAcupunctureArmView), CSize(0, 0), pContext)){//在第一行第一列展示View视图
		return FALSE;
	}

	if (!m_splitter.CreateView(0, 1, RUNTIME_CLASS(CPanelDlg), CSize(0, 0), pContext)){//在第一行第二列展示我们添加的对话框
		return FALSE;
	}
	...
	int width;
	width = rc.Width() - 650;//设置显示对话框的区域大小
