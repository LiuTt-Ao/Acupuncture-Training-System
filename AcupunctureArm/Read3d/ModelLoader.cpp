...
// 将块的内容读入块结构中
int ModelLoader::Read3DSChunk(FILE* fp, Chunk3DS& chunk)
{
...
int ModelLoader::Read3DSString(FILE* fp, char* name, int len /*= 256*/)
{
	int c,i;
...
// 读入子块
int ModelLoader::ReadPercentage(FILE* fp, float& value)
{
	Chunk3DS	chunk;
...
// 读入颜色定义
int ModelLoader::ReadColor(FILE* fp, float& red, float& green, float& blue)
{
	Chunk3DS		chunk;
	long			chunkStart = ftell(fp);
	unsigned char	tmp;
...
// 读入顶点
int ModelLoader::ReadPointArray(CTriObject*	newchild, long fileSize, FILE *fp)
{
...
// 读入多边形
int ModelLoader::ReadFaceArray(CTriObject* newchild, long unsigned fileSize, FILE* fp)
{
...
// 读入对象所用的材质
int ModelLoader::ReadMeshMatGroup(CTriObject* newchild, MaterialDict* matdict, long fileSize, FILE* fp)
{
...
// 读入对象数据
int ModelLoader::ReadTriObject(MaterialDict* matdict, long fileSize, FILE* fp, long triStart, long triSize, char* groupName)
{
...
int ModelLoader::ReadNamedObject(MaterialDict* matdict, long fileSize, long namedStart, long namedSize, FILE* fp)
{
	char		groupName[256];
	Chunk3DS	chunk;
	long		chunkStart;
...
// 读入材质定义，并将其添加到材质库中
int ModelLoader::ReadMatEntry(MaterialDict* matdict, long fileSize, long matStart, long matSize, FILE* fp)
{
	long			chunkStart = ftell(fp);
	Chunk3DS		chunk;
	char			name[256];
		...
		Read3DSChunk(fp, chunk))
	{
		...
		case MAT_SHININESS:
			...
			material.shininess = ((float)percentage) / 255.0f;
			break;
...
// 读入最高级的对象数据
int ModelLoader::ReadMDATA(MaterialDict* matdict, long fileSize, long mdataStart, long mdataSize, FILE* fp)
{
	long			chunkStart = ftell(fp);
	Chunk3DS		chunk;
...
// 读入3DS文件
int ModelLoader::Read3DSFile(long fileSize, long fileStart, long fileLen, FILE* fp)
{
	long			chunkStart = ftell(fp);
	Chunk3DS		chunk;
...
// 验证当前的文件是否是3DS文件
int ModelLoader::Is3DSFile(FILE* fp)
{
	Chunk3DS	chunk;
	long		pos = ftell(fp);
...
// 读入一个3DS文件
BOOL ModelLoader::Reader(char* filename, CTriList* _list)
{
	FILE*		fp;
	long		fileSize;
	Chunk3DS	chunk;
...
int ModelLoader::ReadKFDATA(long fileSize, long kfdataStart, long kfdataSize, FILE * fp)
{
	long		chunkStart = ftell(fp);
	Chunk3DS	chunk;
	short		version;
...
int ModelLoader::ReadKFObjectNode(long fileSize, long nodeStart, long nodeSize, FILE * fp)
{
...
int ModelLoader::ReadKFTrackTag(long fileSize, long tagStart, long tagSize, FILE * fp, char * nodeName, tVector* pivot, Chunk3DS	chunk)
{
	long  numkeys;
	short rflags;
	short trflags;
